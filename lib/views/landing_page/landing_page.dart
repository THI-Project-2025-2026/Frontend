                  ...
                  child: SingleChildScrollView(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        SizedBox(height: sectionSpacing),
                        _LivePreviewSection(isWide: isWide),
                        SizedBox(height: sectionSpacing),
...
class _LivePreviewSection extends StatelessWidget {
  const _LivePreviewSection({required this.isWide});

  final bool isWide;

  @override
  Widget build(BuildContext context) {
    final cardBackground = _themeColor('landing_page.live_preview_background');
    final waveformColor = _themeColor('landing_page.live_preview_waveform');
    final textTheme = Theme.of(context).textTheme;

    return BlocBuilder<LandingPageBloc, LandingPageState>(
      buildWhen: (previous, current) =>
          previous.isLivePreviewPlaying != current.isLivePreviewPlaying,
      builder: (context, state) {
        final bars = state.isLivePreviewPlaying
            ? <double>[68, 42, 80, 54, 76, 44, 70]
            : <double>[16, 10, 22, 12, 18, 8, 14];
          ...
          child: Column(
            ...
            children: [
                ...
                children: [
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        Text(
                          _tr('landing_page.live_preview.title'),
                          style: textTheme.headlineSmall?.copyWith(
                            color: Theme.of(context).colorScheme.onSurface,
                            fontWeight: FontWeight.w700,
                          ),
                        ...
                        const SizedBox(height: 12),
                        Text(
                          _tr('landing_page.live_preview.description'),
                          style: textTheme.bodyMedium?.copyWith(
                            color: Theme.of(
                              context,
                  ...
                  const SizedBox(width: 24),
                  Switch(
                    value: state.isLivePreviewPlaying,
                    activeThumbColor: Theme.of(context).colorScheme.secondary,
                    onChanged: (_) => context.read<LandingPageBloc>().add(
              ...
              Wrap(
                ...
                children: [
                  _PreviewTag(
                    label: _tr('landing_page.live_preview.scene_a'),
                    isActive: !state.isLivePreviewPlaying,
                  ),
                  _PreviewTag(
                    label: _tr('landing_page.live_preview.scene_b'),
                    isActive: state.isLivePreviewPlaying,
                  ),
                ],
              ),
                      ...
                      Expanded(
                        child: AnimatedContainer(
                          ...
                          decoration: BoxDecoration(
                            ...
                            gradient: LinearGradient(
                              colors: [
                                waveformColor.withValues(alpha: 0.65),
                                waveformColor,
                              ],
...
class _PreviewTag extends StatelessWidget {
  const _PreviewTag({required this.label, required this.isActive});
  ...
  Widget build(BuildContext context) {
    ...
    return AnimatedContainer(
      ...
      decoration: BoxDecoration(
        borderRadius: BorderRadius.circular(999),
        border: Border.all(
          color: borderColor.withValues(alpha: isActive ? 0.7 : 0.3),
          width: 1.2,
        ),
        color: borderColor.withValues(alpha: isActive ? 0.12 : 0.05),
      ),
      child: Text(
        label,
        style: Theme.of(context).textTheme.labelLarge?.copyWith(
          color: Theme.of(
            context,
          ).colorScheme.onSurface.withValues(alpha: isActive ? 0.85 : 0.6),
